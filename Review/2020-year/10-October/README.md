...
바로 신청했고, 부랴부랴 발표자료를 만들고, 발표를 진행했다.

저작권 때문에 발표자료를 전부 공개하기는 힘들다.
그래서 몇 가지 핵심적인 내용만 소개해보자면


- 좋은 직장과 좋은 회사는 다르다.
  - 직장은 `나`를 기준으로 해야 한다.
  - 회사는 `타인`, 즉 `외부의 시선`을 기준으로 한다.
- 직장 만족도에 대한 보편적인 카테고리(주관적)
  - 일(업무)
  - 돈(급여)
  - 사람(팀원)
- 위의 세 가지 중 두 가지 이상만 겹치면 대체로 `좋은 직장`이라고 불린다.
- 나에게 있어서 좋은 직장을 판별할 때 우선순위가 되는 것은 다음과 같다.
  - 0순위: **팀원(사람)**
  - 1순위: 자사 서비스가 있는가(일)
  - 2순위: 기술스택(일)
  - 3순위: 잡플래닛(돈/일/사람)
...
그리고 [부스트캠프 공식 블로그](https://m.blog.naver.com/boostcamp_official/222140296542)에 전체적인 내용이 올라왔다.
