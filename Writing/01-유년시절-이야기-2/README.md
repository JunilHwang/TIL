...
그래서 내가 할 수 있던 일은 거짓말을 보태지 않고 위의 코드의 **모든 내용을 그대로 외워버렸다.**
일단 앞서 보여준 `lib.php`를 외웠다. 이 코드가 모든 코드의 근간이 되는 내용이었고, 이 코드를 모르면 다른 코드는 아예 사용이 불가능했다.

거짓말을 보태지 않고 스페이스바, 따옴표, 여백, 인던트, 라인수 등을 포함하여 정말 똑같이 칠 수 있도록 외웠다.
